...
加 --demo 產生線上展示版到 docs/index.html（排除真實報告與本機路徑，供 GitHub Pages）。
"""
import json
import re
import sys
...
DEMO_SAFE_REPORTS = {"2026-07-07-新產品上線時程.md"}  # 合成測試音檔產的，無真實內容
...
for p in sorted((ROOT / "reports").glob("*.md")):
    if DEMO and p.name not in DEMO_SAFE_REPORTS:
        continue
    text = read_md(p)
    m = re.search(r"使用格式：[^\n]*?([^\s（()）:：]+\.md)", text)
    reports.append({
...
data = json.dumps({"formats": formats, "reports": reports,
                   "root": "" if DEMO else str(ROOT), "demo": DEMO},
                  ensure_ascii=False)
...
<style>
  ...
  * { box-sizing:border-box; }
  ...
  .demo-banner { background:var(--accent); color:#fff; padding:12px 32px; font-size:15.5px; }
...
function openDoc(title, md, file){
  ...
  if(file){
    ...
    sub.innerHTML=(DATA.root?
      '<span class="dlg-path">'+esc(DATA.root+"/reports/"+file)+'</span>'+
...
const TOUR=[
 ...
 {t:"跑出你的第一份報告",h:
  ...
  '<p style="font-size:13.5px;color:var(--muted);margin:6px 0 0;">用 ZIP 下載的話，資料夾名稱是 talk-to-minutes-main，一樣拖它。</p>'+
...
if(DATA.demo){
  ...
  b.innerHTML='這是線上展示版：每個功能都可以點，報告是範例。實際使用時，格式與報告都存在你自己的電腦。 '+
    '<a href="https://github.com/Habbo-Shibbo/talk-to-minutes">前往 GitHub 下載 →</a>';
...
outdir = ROOT / ("docs" if DEMO else "app")
outdir.mkdir(exist_ok=True)
out = outdir / "index.html"
out.write_text(TEMPLATE.replace("__DATA__", data), encoding="utf-8")
label = "展示版" if DEMO else ""
print(f"已更新 {out}{label}（格式 {len(formats)} 個、報告 {len(reports)} 份）")
